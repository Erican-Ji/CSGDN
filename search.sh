...
for tau in "${tau_values[@]}"; do
    ...
    for predictor in "${predictor_values[@]}"; do
        for feature_dim in "${feature_dim_values[@]}"; do
                nohup python -u search.py --dataset cotton_80 --tau "$tau" --predictor "$predictor" --feature_dim "$feature_dim" > ./results/cotton_80/CSGDN/tmp_search/cotton_80_tau_"$tau"_predictor_"$predictor"_feature_dim_"$feature_dim".txt 2>&1 &
        done
    done
done
