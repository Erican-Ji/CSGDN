...
class MyGAT(nn.Module):
    ...
    def forward(self, edge_index, x):
        ...
        for _ in range(self.layer_num):
            ...
            x = encoder(x.cpu(), edge_index.cpu())
...
if not os.path.exists(f"./results/{args.dataset}/GAT"):
    os.makedirs(f"./results/{args.dataset}/GAT")
...
for period_name in period:
    ...
    for times in range(5):
        ...
        for epoch in range(1000):
            ...
            # GCN embedding x
            x = model(train_pos_edge_index, x)  # the negative edges are regarded as none edges
    ...
    with open(f"./results/{args.dataset}/GAT/{args.period}_res.txt", "w") as f:
