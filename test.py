...
def generate_graph():
    ...
    for period_name in period:
        ...
        # 保证 train_data.shape[0]: val_data.shape[0]: test_data.shape[0] = 7: 1: 2
        train_count = {}
        for each in train_data:
            if not train_count.get(each[0].item()):
                train_count[each[0].item()] = 1
            else:
                train_count[each[0].item()] += 1

        count = 0
        new_train = torch.tensor([]).to(device)
        add_test = torch.tensor([]).to(device)
        add_val = torch.tensor([]).to(device)
        for key, value in train_count.items():
            count += value
            if value > 2:
                tmp = torch.where(train_data[:, 0] == key)[0]
                add_test = torch.concat((add_test, train_data[tmp[0]].unsqueeze(0)), dim=0)
                if value > 3:
                    add_val = torch.concat((add_val, train_data[tmp[1]].unsqueeze(0)), dim=0)
                    new_train = torch.concat((new_train, train_data[tmp[2:]]), dim=0)
                else:
                    new_train = torch.concat((new_train, train_data[tmp[1:]]), dim=0)
            else:
                new_train = torch.concat((new_train, train_data[torch.where(train_data[:, 0] == key)[0]]), dim=0)

        print(count)
        print(new_train.shape)
        print(add_test.shape)
        print(add_val.shape)

        # add
        train_data = new_train
        test_data = torch.concat((test_data, add_test, add_val), dim=0)
